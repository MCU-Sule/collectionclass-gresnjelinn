...
public class HelloController {
    ...
    public void cariStudent(ActionEvent actionEvent) {
        oList.forEach((data) -> {
            if(data.getNrp() == Integer.parseInt(inputNRP3.getText())) {
                int idx = oList.indexOf(data);
                aList = FXCollections.observableArrayList();
                aList.add(oList.get(idx));
    ...
    public void deleteSelected(ActionEvent actionEvent) {
        int selectedNRP = table.getSelectionModel().getSelectedIndex();
        table.getItems().remove(selectedNRP);
    }
}